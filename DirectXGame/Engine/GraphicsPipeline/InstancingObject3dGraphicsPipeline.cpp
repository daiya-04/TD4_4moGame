...
namespace DaiEngine {
	...
	void InstancingObject3dGraphicsPipeline::Init() {
		...
		D3D12_DESCRIPTOR_RANGE descriptorRangeForObjGPU[1] = {};
		descriptorRangeForObjGPU[0].BaseShaderRegister = 1; //0から始まる
		descriptorRangeForObjGPU[0].NumDescriptors = 1; //数は1つ
		descriptorRangeForObjGPU[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV; //SRVを使う
		descriptorRangeForObjGPU[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND; //Offsetを自動計算
		...
		rootParameters[static_cast<size_t>(RootParameter::kObjectGPU)].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE; //DescriptorTableを使う
		rootParameters[static_cast<size_t>(RootParameter::kObjectGPU)].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL; //PixelShaderで使う
		rootParameters[static_cast<size_t>(RootParameter::kObjectGPU)].DescriptorTable.pDescriptorRanges = descriptorRangeForObjGPU; //Tableの中身の配列を指定
		rootParameters[static_cast<size_t>(RootParameter::kObjectGPU)].DescriptorTable.NumDescriptorRanges = _countof(descriptorRangeForObjGPU); //Tableで利用する数
		...
		rootParameters[static_cast<size_t>(RootParameter::kDirectionLight)].Descriptor.ShaderRegister = 2;  //レジスタ番号1を使う
		...
		//Shaderをコンパイルする
		ComPtr<IDxcBlob> verterShaderBlob = dxCompiler_->ShaderCompile(L"InstancingObject3d.VS.hlsl", L"vs_6_0");
		assert(verterShaderBlob != nullptr);

		ComPtr<IDxcBlob> pixelShaderBlob = dxCompiler_->ShaderCompile(L"InstancingObject3d.PS.hlsl", L"ps_6_0");
	...
	void InstancingObject3dGraphicsPipeline::preDraw(BlendMode blendMode) {
